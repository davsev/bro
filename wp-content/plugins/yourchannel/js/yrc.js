...
jQuery(document).ready(function($){
	...
	function yrcStyle( sel, data ){
		...
		var css = 
			...
			if(data.style.play_icon === 'all') css += sel+' .yrc-video .yrc-thumb a:before{ width:100%; }';
			if(data.style.play_icon === 'hover') css += sel+' .yrc-videoa:hover .yrc-thumb a:before{ width:100%; }';
	...
	YRC.Base.prototype = {		
		...
		'init': function(s, label){
			...
			this.ref.blacklist = (this.data.meta[ label === 'playlists' ? 'blacklisted_playlists' : 'blacklist' ] || '')
		...
		'list': function(){
			...
			var i = this.temp_items.length;
			while(i--){
				var item = this.temp_items[i];
				if( item.snippet.title === 'Private video' || item.snippet.title === 'Deleted video' || bl.indexOf(item.id) > -1) this.temp_items.splice(i, 1);
			}
		...
		sortList: function(){
			...
			} else if(srt==='title_desc') {
				this.temp_items.sort(function(a, b){i = (a.snippet.title < b.snippet.title); return i ? 1 : -1;});
			}
		...
		'listVideos': function(cont, res){
			var core = cont.children('.yrc-core'), first = !this.first_loaded;
		...
		'lstVideos': function(core, res){
			...
			var yc = this, is_large_screen = this.ref.screen === 'yrc-desktop',
				...
				resize_meta = this.theme.style === '__list' && this.theme.thumb[1] === 'adjacent',
			...
			core.find('.yrc-just-listed img').load(function(e){
				...
				if( resize_meta ) figure.next().css('height', is_large_screen ? $(this).height() || 'auto' : 'auto');
			});	
	...
	YRC.Playlists.prototype.afterList = function (){
		var cont = $(this.coresel), core = cont.children('.yrc-core'), yc = this;
	...
	YRC.Styles = $.extend(YRC.Styles, {
		...
		grid: function(yc, core, item){
			...
			var margin_dir = yc.rtl ? 'right' : 'left';
		...
		if(yc.player_mode){	// inline
			...
			if( parseInt(yc.player_mode) === 3){	// at top
				...
				if( yc.data.style.theme.videos.style === '__slides' || yc.data.style.theme.videos.style === '__list' ){
					...
					li.parents('.yrc-sub-section').prepend( YRC.template.player( li, yc, false, vid ) );
				}
			} else {	// inline
				var idx = li.index()+1;
			...
			$('html,body').animate({'scrollTop': $(yc.sel+' .yrc-player').offset().top-ofs}, 'slow');
		} else { // lightbox
	...
	YRC.Player = function(yc, play){
		...
		var player = new YT.Player('yrc-player-frame-'+ yc.id, {
			events: {
				'onReady':function(e){
					if(play && !YRC.iOS) e.target.playVideo();		// to do, uncomment
				},
		...
		YRC.EM.on('yrc.player_state_change', function(e, d){
			...
			try{
				if(d[1].target.m !== player.m) player.pauseVideo();
			} catch(e){
				//console.log(e);
			}
		});
				
		if(player_el.parents('.__grid, .__list, .yrc-single').length){
			YRC.sticky_channel = yc.id;
			//YRC.stickyVideo( true, yc.id );
		}
	...
	YRC.Video.prototype = {
		init: function(){
			...
			this.host.append( YRC.template.player(vid_id, this, false, {id: vid_id, alone:true}, 'yrc-standalone-'+this.id) );
			
			if(window.YT.Player)
				this.player = YRC.Player(this, this.data.meta.autoplay);
						
			YRC.single_videos[vid_id] = this;
			YRC.EM.trigger('yrc.setup', this);
			this.size();
	...
	YRC.template.playlists = '<div class="yrc-section pb-inline">\
								...
								<div class="yrc-playlist-videos yrc-video-list yrc-sub-section"><ul class="yrc-core"></ul></div>\
							</div>';
	...
	YRC.template.content = function( secs ){
		...
		<div class="yrc-content">\
			...
			<div class="yrc-sections">' +
				(secs.uploads ? '<div class="yrc-section pb-inline"><div class="yrc-uploads yrc-video-list yrc-sub-section"><ul class="yrc-core"></ul></div></div>': '') +
	...
	YRC.template.player = function( li, yc, lightbox, vid, frame_id){
			...
			<div class="yrc-player">'
				...
				'<div class="yrc-player-frame">\
					<div class="yrc-player-video">\
						<iframe id="yrc-player-frame-'+ yc.id +'" style="width:100%;height:100%" src="//www.youtube.com/embed/'+v[0]+'?enablejsapi=1&rel=0&origin='+(window.location.origin)+'&modestbranding=1&controls=1" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>\
	...
	YRC.template.playerVideoContent = function( vid, yc ){
		...
		var desc = style.show_desc ? '<div class="yrc-player-video-details">\
										...
										<div class="yrc-player-video-description-more">Show More</div>\
									  </div>' : '';
	...
	YRC.template.video = function( vid, res, style, img, show_desc, i ){
		...
		return '<li class="yrc-video yrc-item-'+ cl +' yrc-item yrc-just-listed" data-video="'+ vid.id +'" data-yi="'+i+'">\
				<figure class="yrc-thumb pb-inline pb-relative">\
					<a href="'+ watch_video + vid.id +'" class="yrc-video-link pb-block" target="_blank">\
						...
						<div class="yrc-name-date yrc-nd-'+style[2]+'">\
							<span class="pb-block yrc-video-title yrc-item-title"><a href="'+ watch_video + vid.id +'" class="yrc-video-link" target="_blank">'+ vid.snippet.title +'</a></span>\
							<span class="yrc-video-date '+( is_live ? 'yrc-live' : '' )+'">'
								+ (is_live ? 'LIVE NOW' : miti( new Date(vid.snippet.publishedAt) )) +
							'</span>'+
	...
	YRC.template.playlistItem = function( item, style, img, show_desc ){
		...
		return '<li class="yrc-playlist-item yrc-item yrc-item-'+cl+'" data-playlist="'+ item.id +'">\
				<figure class="yrc-thumb pb-inline yrc-full-scale"><a href="https://www.youtube.com/playlist?list='+item.id+'" target="_blank"><img src="'+ ( item.thumbs[img] || item.thumbs.large || item.thumbs.medium || item.thumbs.small ) +'"\></a>\
					</figure><div class="pb-inline yrc-item-meta"><div class="pb-block yrc-item-title"><a href="https://www.youtube.com/playlist?list='+item.id+'" target="_blank">'+ item.snippet.title +'</a></div>\
	...
	function cleanPage(){
		...
		$('.entry-content').css({'width': '100%', 'margin': '5em auto'});
	}
	//cleanPage();

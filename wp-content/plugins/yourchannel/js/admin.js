...
jQuery(document).ready(function($){
	...
	$('body').on('click', '#yrc-get-channel-id', function(e){
		...
		ajax(uu, function(re){
			...
			var err = er.responseJSON.error.errors.length ? er.responseJSON.error.errors[0].domain : '',
				msg = '';
			if(err === 'youtube.quota'){
				if(YRC.auth.apikey === YC.dummy.meta.apikey)
					msg = 'This API Key has crossed limit, please get the working API Key from <a href="https://plugin.builders/yourchannel-troubleshooting/" target="_blank">our support page</a>';
				else 
					msg = 'This API Key has crossed limit, please <a href="https://console.developers.google.com/" target="_blank">create a new API Key</a>.';
			} else {
				msg = 'Your API Key is not working, please make sure you followed <a href="https://plugin.builders/yourchannel-setup/" target="_blank">the guide here</a> to create it.';
			}
			if(msg) $('#pbc-form input[name=apikey]').after('<p style="color:red;margin:.25em 0" id="yrc-apikey-error">'+msg+'</p>');
			console.log(er);
		});
	});
	
	$('body').on('change', '#pbc-form input[name=apikey]', function(e){ $('#yrc-apikey-error').remove(); });
	...
	YC.dummy = {
		'meta': {
			...
			'key': 'nw',
			'apikey': 'AIzaSyDvIwlALP79LXiTjET0Vt9IpEWmGbciUAI',
			'cache': 180,
			'channel_uploads': '',
			'onlyonce': '',
			'tag':''
